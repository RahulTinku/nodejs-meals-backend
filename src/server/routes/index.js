const routes = (app, {user, access, role, meal}) => {
  ...
  app.put('/users/:userId/meals/:mealId', access.verifyAuth, user.populateParamsUserId, user.populateTokenUser, role.validateRole('meals', 'update'), meal.updateMeal);
  ...
  app.delete('/users/:userId/meals/:mealId', access.verifyAuth, user.populateParamsUserId, user.populateTokenUser, role.validateRole('meals', 'delete'), meal.removeMeal);
};
